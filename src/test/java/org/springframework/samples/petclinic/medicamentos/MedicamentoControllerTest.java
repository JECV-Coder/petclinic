...
//            .andExpect(model().attributeHasErrors("medicamento"))
  //          .andExpect(model().attributeHasFieldErrors("medicamento", "presentacion"))
    ...
    public void deleteMedicamentos() throws Exception {
        this.mockMvc.perform(get("/medicamento/{medicamentoId}/delete",MEDICAMENTO_ID))
                .andExpect(status().isOk());
    } 
    
    @WithMockUser(username = "admin@admin.com", authorities = {"ADMIN_PRIVILEGE"})
    @Test
    public void showMedicamentos() throws Exception {
        this.mockMvc.perform(get("/medicamento/{medicamentoId}",MEDICAMENTO_ID))
                .andExpect(status().isOk());
    } 
}
